...
public class Solver {
	...
	public static class Plan {
		public Matrix X0;
		public ArrayList<MatrixElement> basis;

		public Plan(Matrix plan, ArrayList<MatrixElement> basis) {
			this.X0 = plan;
			this.basis = basis;
		}
	}
	...
	public static Matrix isolateTransportationProblem(List<Producer> producers, List<ConsumptionPoint> consumers,
			Matrix C) {
		...
		if (totalProduction > totalConsumption) {
			int numOfLastConsumer = consumers.size() - 1;
			...
			consumers.add(fictiveConsumer);

			int numOfRows = C.getRowDimension();
			int newNumOfColumns = C.getColumnDimension() + 1;

			Matrix newCostMatrix = new Matrix(numOfRows, newNumOfColumns);

			for (int i = 0; i < C.getRowDimension(); i++)
				for (int j = 0; j < C.getColumnDimension(); j++)
					newCostMatrix.set(i, j, C.get(i, j));

			return newCostMatrix;
		} else {
			int numOfLastProducer = producers.size() - 1;

			int id = producers.get(numOfLastProducer).getId() + 1;
			String name = "fictive";
			double production = totalConsumption - totalProduction;
			double costPerUnit = 0;

			Producer fictiveProducer = new Producer(id, name, production, costPerUnit);
			fictiveProducer.setType(Type.FICTIVE);

			producers.add(fictiveProducer);

			int newNumOfRows = C.getRowDimension() + 1;
			int numOfColumns = C.getColumnDimension();

			Matrix newCostMatrix = new Matrix(newNumOfRows, numOfColumns);
